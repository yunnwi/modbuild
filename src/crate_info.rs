...
pub fn get_crate_name(mod_path: &PathBuf) -> Result<String, String> {
    ...
    let manifest_path = mod_path.join("Cargo.toml")
      .canonicalize()
    ...
    let pkg = packages.iter().find(|p| {
        p["manifest_path"].as_str().map(|s| s == manifest_str).unwrap_or(false)
    }).or_else(|| packages.first()).ok_or("No package found")?;
...
pub fn ensure_cdylib(mod_path: &PathBuf) -> Result<(), String> {
    ...
    let has_cdylib = targets.iter().any(|t| {
        t["kind"].as_array().map(|ks| ks.iter().any(|k| k == "cdylib")).unwrap_or(false)
    });
    if !has_cdylib {
